...
import type { Expose, MutashabihExemple } from '../../types';

/**
 * Article de fond « Comprendre les textes équivoques » (à lire en premier).
 * Tout est éditable en base : la prose et le bloc « verset fondateur » viennent
 * de `exposes` (slug comprendre-textes-equivoques) ; les cartes « subtilité de la
 * langue » de `mutashabih_exemples`.
 */
export const ComprendreEquivoques: React.FC = () => {
  ...
  useEffect(() => {
    ...
    ]).then(([e, ex]) => { setExpose(e); setExemples(ex); }).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="min-h-screen bg-ground">
      <main className="max-w-3xl mx-auto px-5 py-7 pb-16">
        ...
        {/* Prose de l'article */}
        {expose?.contenu_md ? (
          <div className="mt-7"><Markdown>{expose.contenu_md}</Markdown></div>
        ) : (
          <p className="mt-7 text-muted italic">Article en cours de rédaction.</p>
        )}
