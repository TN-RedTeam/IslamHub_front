...
export default defineConfig({
  ...
  plugins: [
    react(),
    VitePWA({
      // Mise à jour automatique : dès qu'un nouveau build est déployé, le SW
      // se met à jour et prend la main → jamais de version périmée en cache
      // (c'est CE point qui avait « cassé » le site la dernière fois).
